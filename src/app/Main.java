...
public final class Main {
    ...
    private static void boot(AppSettings settings,
        ...
        linkRef.set(link);
        startDetection(link, log, linkRef, clientRef, dumpCtlRef, shutdownDone, settings);
    }
    ...
    private static void startDetection(SerialLink link,
        ...
        Thread worker = new Thread(
                () -> runDetectAndEntryLoop(link, log, linkRef, clientRef, dumpCtlRef,
                        shutdownDone, progress, settings),
        ...
        while (true) {
            ...
            SwingUtilities.invokeLater(() -> {
                ...
                showPlaceholderMainWindow(link, log, finalState,
                        linkRef, clientRef, dumpCtlRef, shutdownDone, settings);
            });
            return;
        }
    }
    ...
    private static int defaultDumpAddressFromSettings(AppSettings settings) {
        String h = settings.getLastAddress();
        if (h == null || h.length() != 4) {
            return 0;
        }
        try {
            return HexUtils.parse4Hex(h);
        } catch (IllegalArgumentException e) {
            return 0;
        }
    }

    private static int defaultDumpBytesFromSettings(AppSettings settings) {
        int b = settings.getLastBytes();
        if (b <= 0) {
            return 16;
        }
        return Math.min(b, DumpPromptDialog.MAX_BYTES);
    }

    private static int defaultBurstFromSettings(AppSettings settings) {
        int b = settings.getLastBurst();
        if (b < DumpPromptDialog.MIN_BURST || b > DumpPromptDialog.MAX_BURST) {
            return AppSettings.DEFAULT_LAST_BURST;
        }
        return b;
    }
    ...
    private static void showPlaceholderMainWindow(SerialLink link,
        ...
        dumpBtn.addActionListener(e -> {
            ...
            DumpPromptDialog.Result r = DumpPromptDialog.showDialog(frame,
                    defaultDumpAddressFromSettings(settings),
                    defaultDumpBytesFromSettings(settings),
                    defaultBurstFromSettings(settings));
            if (r == null) {
                return;
            }
            settings.setLastAddress(HexUtils.toHex4(r.addr()));
            settings.setLastBytes(r.bytes());
            settings.setLastBurst(r.burst());
            settings.flush();
            startDumpViaController(controller, r.addr(), r.bytes(), r.burst(), session,
    ...
    private static void startDumpViaController(DumpController controller,
        ...
        controller.start(addr, bytes, burst, new DumpController.Listener() {
            @Override
