 ...
 *       {@link PK232Client#readMMBurst(int)} call (Q3 answer 2026-04-21);
 *       (burst {@code 1} issues one {@code MM} per await, same as the
 *       legacy {@link PK232Client#readOneByte()} cadence);
...
public final class DumpController {
     ...
     *
     * @param burst {@code MM} batch size per round-trip ({@code 1}..{@code 99});
     *              {@code 1} preserves one-send-one-reply behavior.
     * @throws IllegalArgumentException addr / bytes / burst / addr+bytes out of range
     * @throws IllegalStateException    another dump is already running
     *                                  (EDT must gate the Dump button per C2)
     */
    public void start(int addr, int bytes, int burst, Listener listener) {
        ...
        if (burst < 1 || burst > 99) {
            throw new IllegalArgumentException("burst must be 1..99: " + burst);
        }
        ...
        Thread w = new Thread(
                () -> dumpLoop(addr, bytes, burst, buffer, listener),
     ...
     * observes this between {@link PK232Client#readMMBurst(int)} calls and
    ...
    private void dumpLoop(int addr, int bytes, int burst, byte[] buffer, Listener listener) {
        ...
        try {
            ...
            while (bytesDone < bytes) {
                ...
                int burstThis = Math.min(burst, bytes - bytesDone);
                int[] values = client.readMMBurst(burstThis);
                for (int j = 0; j < burstThis; j++) {
                    buffer[bytesDone] = (byte) values[j];
                    bytesDone++;
     ...
     * is a no-op in practice because {@link #start(int, int, int, Listener)}
