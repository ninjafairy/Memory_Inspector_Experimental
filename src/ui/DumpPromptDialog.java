...
/**
 * Modal ADDRESS + BYTES + BURST prompt used by the M4 Dump button and, in M5+,
 ...
 *       per truenorth C4.</li>
 *   <li>BURST: decimal {@code 1}..{@code 99} (wire behavior is wired
 *       separately; the dialog only captures and returns the value).</li>
 * </ul>
 *
...
public final class DumpPromptDialog extends JDialog {
    ...
    public static final int MAX_BURST     = 99;

    /** Result of a successful OK — ADDR, BYTES, and BURST validated (+ BYTES clamped per C4). */
    public record Result(int addr, int bytes, int burst) { }

    private final JTextField addrField  = new JTextField(6);
    private final JTextField bytesField = new JTextField(6);
    private final JTextField burstField = new JTextField(3);

    private Result result;

    /**
     * Convenience factory used by callers that just want to show-and-await.
     * {@code parent} may be {@code null}. Defaults populate the fields
     * so a quick retry doesn't require re-typing.
     */
    public static Result showDialog(Window parent, int defaultAddr, int defaultBytes, int defaultBurst) {
        DumpPromptDialog d = new DumpPromptDialog(parent, defaultAddr, defaultBytes, defaultBurst);
        d.setVisible(true);
        return d.result;
    }

    private DumpPromptDialog(Window parent, int defaultAddr, int defaultBytes, int defaultBurst) {
        ...
        ((PlainDocument) burstField.getDocument()).setDocumentFilter(new DecimalDigitsFilter(2));
        ...
        burstField.setText(Integer.toString(clampBurst(defaultBurst)));
        ...
        gc.gridx = 0;
        gc.gridy = 2;
        form.add(new JLabel("Burst (1..99):"), gc);
        gc.gridx = 1;
        form.add(burstField, gc);
    ...
    private void onOk() {
        ...
        String burstStr = burstField.getText().trim().replaceFirst("^0+(?!$)", "");
        int burstInt;
        try {
            burstInt = Integer.parseInt(burstStr);
        } catch (NumberFormatException nfe) {
            JOptionPane.showMessageDialog(this,
                    "Burst must be a decimal integer from " + MIN_BURST + " to " + MAX_BURST + ".",
                    "Invalid Burst", JOptionPane.WARNING_MESSAGE);
            burstField.requestFocusInWindow();
            burstField.selectAll();
            return;
        }
        if (burstInt < MIN_BURST || burstInt > MAX_BURST) {
            JOptionPane.showMessageDialog(this,
                    "Burst must be from " + MIN_BURST + " to " + MAX_BURST + ".",
                    "Invalid Burst", JOptionPane.WARNING_MESSAGE);
            burstField.requestFocusInWindow();
            burstField.selectAll();
            return;
        ...
        this.result = new Result(addr, bytesInt, burstInt);
        dispose();
    }
    ...
    private static int clampBurst(int burst) {
        if (burst < MIN_BURST) {
            return MIN_BURST;
        }
        return Math.min(burst, MAX_BURST);
    }
