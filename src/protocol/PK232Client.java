 ...
 *       Each block is either correlated to the pending
 *       {@link #sendAndAwait(HostBlock, long)} / {@link #readMMBurst(int)}
 *       request (if it is a {@code 0x4F} block and a request is pending)
 *       or routed to the
 ...
 * A fair {@link ReentrantLock} serializes {@code sendAndAwait} and
 * {@code readMMBurst} — at most one logical caller at a time (truenorth A3,
 * depth = 1). Pending state is stored in a single volatile slot and handed
 * off via a {@link CountDownLatch} (one or many correlated {@code 0x4F}
 * responses). Timeout throws {@link ProtocolTimeoutException}
...
public final class PK232Client implements AutoCloseable {
    ...
    /**
     * Valid MM read replies observed for the in-flight MM burst (reset to 0
     * at burst start/end). Updated on the protocol thread; use
     * {@link #getBurstRepliesReceived()} to read.
     */
    private final AtomicInteger burstRepliesReceived = new AtomicInteger(0);
    ...
    /**
     * How many valid MM read response packets have been accepted for the
     * current burst so far (0 after each burst completes or before the next
     * {@link #readOneByte()} / {@link #readMMBurst(int)}).
     */
    public int getBurstRepliesReceived() {
        return burstRepliesReceived.get();
    }
    ...
    /**
     * Issues {@code count} {@code MM} frames back-to-back (no await between
     * sends), then waits until all {@code count} correlated {@code 0x4F}
     * replies have arrived and {@link #getBurstRepliesReceived()} equals
     * {@code count} (valid MM payloads only) before returning — so callers
     * cannot begin the next burst until this method returns. {@code count}
     * must be {@code 1..99}. {@code count == 1} delegates to
     * {@link #readOneByte()} (same lock + correlation semantics as a single
     * {@code sendAndAwait}).
     */
    public int[] readMMBurst(int count) throws IOException, InterruptedException {
        if (count < 1 || count > 99) {
            throw new IllegalArgumentException("count must be 1..99: " + count);
        }
        if (count == 1) {
            return new int[] { readOneByte() };
        }
        if (!isRunning()) {
            throw new IOException("PK232Client is not running");
        }
        ...
        HostBlock req = HostBlock.ofAscii(HostBlock.CTL_GLOBAL_COMMAND, "MM");
        byte[] framed = HostCodec.encode(req.ctl(), req.payloadCopy());
        String mnem = deriveMnemonic(req, "CMD");
        long timeoutMs = (long) DEFAULT_TIMEOUT_MS * count;

        sendLock.lockInterruptibly();
        try {
            burstRepliesReceived.set(0);
            PendingRequest p = new PendingRequest(count);
            pending = p;
            for (int i = 0; i < count; i++) {
                log.log(PacketLogger.Direction.TX, mnem, framed, 0, framed.length);
                link.write(framed);
            }
            HostBlock[] responses = p.awaitMulti(timeoutMs);
            if (responses == null) {
                throw new ProtocolTimeoutException(
                        "incomplete MM burst (" + count + " TX) within " + timeoutMs + " ms");
            }
            verifyMmBurstRepliesComplete(count);
            int[] out = new int[count];
            for (int i = 0; i < count; i++) {
                out[i] = parseMMPayload(responses[i]);
            }
            log.logRaw("PK232Client: MM dump burst finished; MM reply packets counted="
                    + count + " this burst (counter reset on exit)");
            return out;
        } finally {
            burstRepliesReceived.set(0);
            pending = null;
    ...
    private void dispatch(HostBlock block) {
        ...
        if (p != null && block.isGlobalCommand()) {
            ...
            if (p.expectsAnotherCorrelatedReply()) {
                try {
                    parseMMPayload(block);
                    int n = burstRepliesReceived.incrementAndGet();
                    if (p.isMultiMmBurst()) {
                        log.logRaw("PK232Client: MM dump burst replies received=" + n);
                    }
                } catch (IOException ignored) {
                    // not a decodable MM read payload — do not bump counter
                }
            }
            p.complete(block);
            return;
    ...
    /**
     * Ensures every MM in the current burst produced a valid MM read reply
     * ({@link #getBurstRepliesReceived()} equals {@code expected}) before the
     * next burst may be issued. Called after the correlation latch has already
     * satisfied the wait for {@code expected} frames.
     */
    private void verifyMmBurstRepliesComplete(int expected) throws IOException {
        int got = burstRepliesReceived.get();
        if (got != expected) {
            throw new IOException(
                    "MM burst: expected " + expected + " valid MM repl"
                            + (expected == 1 ? "y" : "ies")
                            + " (burstRepliesReceived), got " + got);
        }
    }

    /**
     * Correlates one or more consecutive {@code 0x4F} responses to a single
     * logical TX sequence held under {@link #sendLock}.
     */
    private static final class PendingRequest {
        private final CountDownLatch latch;
        private final int expectedSlots;
        private volatile HostBlock singleResponse;
        private final HostBlock[] multiResponses;
        private final AtomicInteger multiSlot = new AtomicInteger(0);

        PendingRequest(int expectedResponses) {
            if (expectedResponses < 1) {
                throw new IllegalArgumentException("expectedResponses: " + expectedResponses);
            }
            this.expectedSlots = expectedResponses;
            this.latch = new CountDownLatch(expectedResponses);
            this.multiResponses = expectedResponses > 1 ? new HostBlock[expectedResponses] : null;
        }

        /** {@code true} while at least one correlated reply is still outstanding. */
        boolean expectsAnotherCorrelatedReply() {
            return latch.getCount() > 0;
        }

        /** Rapid-fire multi-{@code MM} burst (vs single {@code readOneByte}). */
        boolean isMultiMmBurst() {
            return expectedSlots > 1;
        }

        void complete(HostBlock block) {
            if (multiResponses != null) {
                int i = multiSlot.getAndIncrement();
                if (i < multiResponses.length) {
                    multiResponses[i] = block;
                    latch.countDown();
                }
            } else {
                singleResponse = block;
                latch.countDown();
            }
        }

        HostBlock awaitOne(long timeoutMs) throws InterruptedException {
        ...
        HostBlock[] awaitMulti(long timeoutMs) throws InterruptedException {
            if (!latch.await(timeoutMs, TimeUnit.MILLISECONDS)) {
                return null;
            }
